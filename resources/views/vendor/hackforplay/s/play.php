...
$stmt	= $dbh->prepare('SELECT s."ID",s."UserID",s."Mode",s."Title",s."Explain",s."State",s."Playcount",s."Src",s."Thumbnail",s."SourceID",s."NoRestage",u."Nickname",source."Title" AS SourceTitle,script."RawCode" FROM "Stage" AS s LEFT OUTER JOIN "User" AS u ON s."UserID"=u."ID" LEFT OUTER JOIN "Stage" AS source ON s."SourceID"=source."ID" LEFT OUTER JOIN "Script" AS script ON s."ScriptID"=script."ID" WHERE s."ID"=:stageid');
