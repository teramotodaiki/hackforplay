...
* load.php
* Input: pavilion_id
* Output: no-session, invalid-input, uncertified, database-error, result:JSON
*/
...
// パビリオンの存在確認
$pavilion_id	= filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
if (!$pavilion_id) {
	exit('invalid-input');
}
...
$stmt_qu->bindValue(":published", TRUE, PDO::PARAM_INT);
$stmt_qu->execute();
...
while ($quest = $stmt_qu->fetch(PDO::FETCH_ASSOC)) {
	...
	$stmt_num_w->bindValue(":clear", TRUE, PDO::PARAM_INT);
	$stmt_num_w->execute();
